...
.
├── SQL_pipeline.ipynb          # (Объединенный ETL-пайплайн)
├── schema.sql                  # (SQL-код для создания таблиц)
├── check_data.ipynb            # (Скрипт для аудита и проверки качества данных)
├── .gitignore                  # (Игнорируемые файлы: ключи, кэш)
